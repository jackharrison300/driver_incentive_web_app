...
cognito_client_id=$(aws cloudformation describe-stacks --query "Stacks[?StackName=='team25prod'][].Outputs[?OutputKey=='UserPoolClientId'].OutputValue" --output text)
cognito_userpool_id=$(aws cloudformation describe-stacks --query "Stacks[?StackName=='team25prod'][].Outputs[?OutputKey=='UserPoolId'].OutputValue" --output text)
api_url=$(aws cloudformation describe-stacks --query "Stacks[?StackName=='team25prod'][].Outputs[?OutputKey=='API'].OutputValue" --output text)
...
# s3 upload
sbucket=$(aws cloudformation describe-stacks --query "Stacks[?StackName=='team25prod'][].Outputs[?OutputKey=='StaticBucket'].OutputValue" --output text)
